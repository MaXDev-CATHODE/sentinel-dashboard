...
import { 
  ...
  CheckCircle, Briefcase, ExternalLink
} from 'lucide-react';
...
const App = () => {
  ...
  // Automatic "Live" Log generation
  useEffect(() => {
    const interval = setInterval(() => {
      const actions = [
        "Useme Scraper: Found 12 new listings in 'Python'",
        "Security: Blocked SQL Injection attempt from IP 192.168.x.x",
        "OLX Harvester: Parsing page 2/5...",
        "System: Optimal memory usage (45%)",
        "Sentinel: Vunerability scan completed - 0 issues",
        "Optimization: Cache hit ratio 98.2%"
      ];
        ...
        <aside className="w-64 bg-sentinel-panel/60 backdrop-blur-xl border border-sentinel-dim/20 rounded-2xl flex flex-col p-6 shadow-2xl">
          <div className="flex items-center gap-3 mb-10">
            ...
            <div>
              ...
              <span className="text-xs text-sentinel-dim uppercase tracking-[0.2em]">Command Hub</span>
            </div>
          </div>
          ...
          <nav className="flex-1 space-y-2">
            {[
              ...
              { id: 'jobs', icon: Briefcase, label: 'Job Feed' },
              { id: 'security', icon: Lock, label: 'Security Radar' },
              { id: 'analytics', icon: BarChart3, label: 'Analytics' },
              { id: 'terminal', icon: Terminal, label: 'System Console' },
            ].map(item => (
              <button
                key={item.id}
          ...
          <div className="mt-auto pt-6 border-t border-sentinel-dim/20">
            ...
            <p className="text-xs text-sentinel-dim">v2.4.0 (Stable)</p>
          </div>
        </aside>
        ...
        <main className="flex-1 flex flex-col gap-4 overflow-hidden">
          ...
          <header className="h-16 bg-sentinel-panel/60 backdrop-blur-md rounded-2xl border border-sentinel-dim/20 flex items-center justify-between px-6 shadow-lg">
            <div className="flex items-center gap-4 text-sentinel-dim">
              <Globe size={18} />
              <span>EU-West-1 Server</span>
              <span className="w-px h-4 bg-sentinel-dim/20" />
              <span className="text-sentinel-success flex items-center gap-2">
                <Wifi size={14} /> Latency: 12ms
              </span>
            </div>
            <div className="flex items-center gap-4">
              <button onClick={handleDemoClick} className="p-2 hover:bg-sentinel-dim/10 rounded-lg transition-colors relative">
                <AlertTriangle size={20} className="text-sentinel-warning" />
          ...
          <div className="flex-1 grid grid-cols-3 grid-rows-2 gap-4">
              ...
              <div className="h-[200px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={performanceData}>
                    ...
                    <YAxis hide />
                    <Tooltip 
                      contentStyle={{ backgroundColor: '#13131f', borderColor: '#7c3aed' }}
                      itemStyle={{ color: '#fff' }}
                    />
                    <Area type="monotone" dataKey="load" stroke="#7c3aed" strokeWidth={3} fillOpacity={1} fill="url(#colorLoad)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>

            {/* Widget 2: Security Status */}
            <div className="bg-sentinel-panel/40 backdrop-blur-md rounded-2xl border border-sentinel-dim/10 p-6 flex flex-col justify-center items-center relative group hover:border-sentinel-error/50 transition-colors">
              <div className="absolute inset-0 bg-gradient-to-tr from-sentinel-error/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
              <div className="relative w-40 h-40">
                ...
                <div className="absolute w-full h-[2px] bg-sentinel-error/50 top-1/2 left-0 animate-[spin_3s_linear_infinite]" />
                <Lock className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white w-10 h-10" />
              </div>
              <h3 className="mt-4 text-xl font-bold text-white">System Secure</h3>
              <p className="text-sentinel-success text-sm flex items-center gap-1 mt-1">
                <CheckCircle size={14} /> Threat Level: Low
              </p>
            </div>
              ...
              <div className="flex-1 overflow-hidden font-mono text-xs space-y-3 mask-image-b-0">
                {logs.map((log, i) => (
                  <div key={i} className={`p-2 rounded border-l-2 pl-3 animate-in fade-in slide-in-from-right-4 duration-500
                    ${log.includes('Security') ? 'border-sentinel-error bg-sentinel-error/10 text-red-200' : 
                      log.includes('Useme') ? 'border-sentinel-accent bg-sentinel-accent/10 text-purple-200' : 
                      'border-sentinel-success bg-sentinel-success/10 text-green-200'}`}>
                    {log}
                  </div>
                ))}
              </div>
            ...
            <div className="col-span-2 bg-sentinel-panel/40 backdrop-blur-md rounded-2xl border border-sentinel-dim/10 p-6 relative overflow-hidden">
               <h3 className="text-sm uppercase tracking-wider text-sentinel-dim mb-4 flex items-center gap-2">
                <Layers size={16} /> Active Operations
              </h3>
              <div className="grid grid-cols-2 gap-4">
                {[
                  { name: 'Parleto Harvester', status: 'Running', load: '12%' },
                  { name: 'Wordpress Hardening_V2', status: 'Deploying', load: '45%' },
                  { name: 'SEO Crawler Bot', status: 'Idle', load: '0%' },
                  { name: 'Data Pipeline Main', status: 'Processing', load: '89%' },
                ].map((bg, i) => (
                  <div key={i} className="bg-sentinel-bg/50 p-4 rounded-xl border border-sentinel-dim/10 flex justify-between items-center group hover:bg-white/5 transition-colors cursor-pointer" onClick={handleDemoClick}>
                    ...
                    <div className="text-right">
                      <div className="text-xs text-sentinel-dim">CPU Load</div>
            ...
            <div className="text-center space-y-6">
              ...
              <div>
                ...
                <p className="text-sentinel-dim">
                  This is a portfolio demonstration. Advanced control features and live database connections are disabled in this public preview.
                </p>
              </div>

              <div className="bg-sentinel-bg/50 p-4 rounded-xl border border-white/5 text-left text-sm space-y-2">
                <div className="flex justify-between">
                  <span className="text-sentinel-dim">Session:</span>
                  <span className="text-white font-mono">DEMO_GUEST_001</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sentinel-dim">Access Level:</span>
                  <span className="text-sentinel-warning font-mono">READ_ONLY</span>
                </div>
              </div>
